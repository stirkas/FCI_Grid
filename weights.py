...
#TODO: Figure out how to incorporate these in BOUT? What does BOUT++ already do?
...
def calc_perp_weights(xinds, zinds, in_mask):
    ...
    Returns
    -------
    w : float array, shape (N,4)
        Weights in the **standard bilinear order**:
        w[:,0] = w00 -> (i0,   j0  )  # lower-left
        w[:,1] = w10 -> (i0+1, j0  )  # lower-right
        w[:,2] = w01 -> (i0,   j0+1)  # upper-left
        w[:,3] = w11 -> (i0+1, j0+1)  # upper-right
    """
    #TODO: Throw exceptions? made for ghost points so always should be in the grid anyway...
    x0 = np.floor(xinds).astype(int)
    z0 = np.floor(zinds).astype(int)
