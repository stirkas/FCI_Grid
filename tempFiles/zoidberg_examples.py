...
def dommaschk(nx=68,ny=16,nz=128, C=None, period = 1,
              xcentre=1.0, xcentre_inner=None,
              Btor=1.0,
              a=0.1, a_inner = None,
              curvilinear=True, rectangular=False,
              fname='Dommaschk.fci.nc',
              curvilinear_inner_aligned=True, curvilinear_outer_aligned=True,
              npoints=421,
              show_maps=False,
              calc_curvature=True,
              smooth_curvature=False,
              return_iota=True, write_iota=False,
              dagp_coefficients=True):

    if xcentre_inner is None:
        xcentre_inner = xcentre
    if a_inner is None:
        a_inner = a / 2
    ...
    if C is None:
        ...
        C = np.zeros((16,7,4))
        C[0,1,0] =  9.43240e-3 #0.0094324
        C[0,3,0] =  4.01024e-2 #0.0401024
        C[0,5,0] = -2.99792e1
        C[0,2,2] =  2.55341e-1
        C[0,4,2] = -1.40220e1
        C[0,6,2] = -3.87726e3
        C[5,1,0] =  1.12792e-2 #0.011279
        C[5,3,0] = -4.15263e0
        C[5,5,0] = -1.35852e2
        C[5,0,1] = -1.21413e-2
        C[5,2,1] =  1.25878e0
        ...
        raise ValueError(f"Unrecognised option C = {C}")
    
    yperiod = 2*np.pi / period
    field = zb.field.DommaschkPotentials(C, R_0=xcentre_inner, B_0=Btor)
    # Define the y locations
    ycoords = np.linspace(0.0, yperiod, ny, endpoint=False)
    start_r = xcentre_inner + a_inner
    start_z = 0.
    ...
    if rectangular:
        ...
        print ("Making curvilinear poloidal grid")
        inner = zb.rzline.shaped_line(R0=xcentre_inner, a=a_inner, elong=0, triang=0.0, indent=0, n=npoints)
        outer = zb.rzline.shaped_line(R0=xcentre, a=a, elong=0, triang=0.0, indent=0, n=npoints)
    ...
    if dagp_coefficients:
        # Calculate dagp_fv coefficients
        from zoidberg import stencil_dagp_fv
        dagp_vars = stencil_dagp_fv.doit(poloidal_grid)
...
def calc_curvilinear_curvature(fname, field, grid, maps):
        ...
        for y in np.arange(0,B.shape[1]):
            ...
            GR[:,y,:] = field.Bxfunc(R[:,y,:], Z[:,y,:], y)/((B[:,y,:])**2)
            GZ[:,y,:] = field.Bzfunc(R[:,y,:], Z[:,y,:], y)/((B[:,y,:])**2)
            Gphi[:,y,:] = field.Byfunc(R[:,y,:], Z[:,y,:], y)/((B[:,y,:])**2)
